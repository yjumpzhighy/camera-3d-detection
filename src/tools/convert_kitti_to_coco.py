...
DATA_PATH = '/home/zuyuan/Data/kitti_tracking'
SQUENCE_RAW_FILE_PATH = '/raw_data'
SQUENCE_JSON_FILE = '/label_2/training'

#SPLITS = ['train_half', 'val_half', 'train', 'test']
SPLITS = ['train', 'test']
train_percent = 90
val_percent = 10
raw_video_name_list = os.listdir(DATA_PATH + SQUENCE_JSON_FILE)
#random.shuffle(raw_video_name_list)
train_num = int(train_percent/100 * len(raw_video_name_list))
val_num = len(raw_video_name_list) - train_num
VIDEO_SETS = {'train': range(train_num), 'train_half': range(train_num),
              'val_half': range(train_num, train_num+val_num),
              'test': range(train_num, train_num+val_num)}
...
#cats = ['SEDAN', 'TRUCK', 'PEDESTRIAN',  'BUS', 'BICYCLE', 'TRAFFIC_CONE_BUCKET',
#        'ROAD_BARRIER',  'ANIMAL', 'EMERGENCYVEHICLE', 'TRAILER', 'VAN']
cats = ['Car', 'Van', 'Truck', 'Pedestrian', 'Cyclist']
...
if __name__ == '__main__':
  for split in SPLITS:
    ret = {'images': [], 'annotations': [], "categories": cat_info,
           'videos': []}
    num_images = 0
    for i in VIDEO_SETS[split]:  #each video sequence
      image_id_base = num_images
      video_name = raw_video_name_list[i][:-4] #'{:04d}'.format(i)
      ret['videos'].append({'id': i + 1, 'file_name': video_name})
      ann_dir = 'train'  if not ('test' in split) else split
      video_path = DATA_PATH + SQUENCE_RAW_FILE_PATH + '/{}ing/image_2/{}'.format(ann_dir, video_name)
      #image_name = DATA_PATH + SQUENCE_RAW_FILE_PATH + '/{}ing/image_2/{}.png'.format(ann_dir, video_name)
      calib_path = DATA_PATH + '/calib/{}ing/calib/'.format(ann_dir) + '{}.txt'.format(video_name)
      ...
      ann_path = DATA_PATH + SQUENCE_JSON_FILE + '/{}.txt'.format(video_name)
      ...
      for ann_ind, txt in enumerate(anns):  #iterate each bbox
        tmp = txt[:-1].split(' ')
        frame_id = int(tmp[0])
        frame_path = video_path + '/{:06d}.png'.format(frame_id)

        track_id = int(tmp[1])
        if tmp[2] not in cat_ids:
          continue
        ...
        #x,y,z in camera coordinates in meters
        ...
        if False:
          # show image with bbox draw on it
          show_img = cv2.imread(frame_path)
          cv2.rectangle(show_img,(int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), \
                        (0,255,0), 2)
          image = cv2.circle(show_img, (int(amodel_center[0]), int(amodel_center[1])), \
